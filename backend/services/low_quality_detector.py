...
# Create file handler for structured JSON logs
from pythonjsonlogger import jsonlogger
log_handler = logging.FileHandler(log_dir / "low_quality_downloads.log", encoding='utf-8')
formatter = jsonlogger.JsonFormatter('%(asctime)s %(levelname)s %(message)s')
log_handler.setFormatter(formatter)
low_quality_logger.addHandler(log_handler)
low_quality_logger.setLevel(logging.INFO)


def is_file_acceptable(file_path: str, log: logging.Logger) -> bool:
    """
    Checks if a downloaded audio file meets quality standards.
    
    Criteria:
    1. File size must be at least 2.0 MB.
    2. Duration must be at least 90.0 seconds.
    
    Args:
        file_path (str): Path to the downloaded audio file.
        log (logging.Logger): Session logger for general logging.
