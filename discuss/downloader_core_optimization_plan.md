# DownloaderCore 代码结构优化方案

## 现状分析

当前 `downloader_core.py` 文件存在以下问题：

1. **文件过大**：约500行代码，超出300行的限制
2. **类职责过多**：
   - `MusicDownloader` 同时处理API交互、下载、元数据嵌入等功能
   - `DownloaderCore` 负责搜索、下载协调、信息补全等多重职责
3. **代码重复**：元数据处理中有重复的FLAC/MP3处理逻辑
4. **技术栈混用**：同时使用 `requests` 和 `httpx`
5. **硬编码配置**：平台列表、质量排序等配置硬编码在代码中

## 优化目标

1. **符合架构规范**：将文件拆分为多个小于300行的模块
2. **单一职责原则**：每个类和模块只负责一个明确的功能
3. **提高可维护性**：通过模块化降低代码耦合度
4. **统一技术栈**：全面使用异步 `httpx` 客户端
5. **配置化管理**：将硬编码配置提取为常量
...
#### 新增模块：
1. `backend/services/metadata_handler.py` - 元数据处理服务
2. `backend/services/platform_service.py` - 平台映射和搜索服务
3. `backend/services/quality_checker.py` - 质量检测服务
4. `backend/services/qq_music_service.py` - QQ音乐信息补全服务
5. `backend/constants/download_constants.py` - 下载相关常量

#### 重构后的模块职责：

**`downloader_core.py`（重构后）**：
- 保留 `MusicDownloader` 和 `DownloaderCore` 类的核心结构
- 专注协调下载流程
- 移除具体的元数据处理、平台映射、质量检测等实现
...
**文件**: `backend/constants/download_constants.py`
...
**文件**: `backend/services/metadata_handler.py`
...
**文件**: `backend/services/platform_service.py`
...
from constants.download_constants import PLATFORM_MAPPING, PLATFORM_SEARCH_ORDER
from services.downloader_core import APIError
...
**文件**: `backend/services/quality_checker.py`
...
from pathlib import Path
from mutagen import File
from constants.download_constants import MIN_FILE_SIZE_MB, MIN_DURATION_SECONDS
...
**文件**: `backend/services/qq_music_service.py`
...
**文件**: `backend/services/downloader_core.py`（重构后）
...
from services.qq_music_service import QQMusicService
from constants.download_constants import QUALITY_ORDER
...
1. 在 `download_queue_manager.py` 中更新导入语句
2. 在 `download_service.py` 中更新导入语句
3. 在 `low_quality_detector.py` 中可能需要调整与新质量检测模块的集成
...
1. **符合架构规范**：每个文件都控制在300行以内
2. **职责分离**：每个模块只负责一个明确的功能
3. **可维护性提升**：代码结构更清晰，易于理解和修改
4. **可测试性增强**：模块化设计便于编写单元测试
5. **技术栈统一**：全面使用异步httpx客户端
6. **配置化管理**：硬编码配置提取为常量模块